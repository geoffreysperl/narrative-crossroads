---
layout: default
title: Downloads
nav_order: 8
---

# Downloads

Ready-to-use modules with character profiles, threshold moments, lens prompts, and implementation guidelines.

---

## Core System

| Resource | Description |
|:---------|:------------|
| [Narrative Crossroads Solo Framework SRD](/downloads/Narrative_Crossroads_Framework.pdf) | The System Reference Document — the reusable core of the system, in print form. The [web version]({% link solo-srd.md %}) is canonical. |

---

## Classroom Handouts

| Resource | Description |
|:---------|:------------|
| [Character Frame Card](/downloads/NC_Character_Frame_Card.pdf) | Blank cards, two to a page — half-sheets, one cut. The one page students write on. |

---

## Literary Modules

| Text | Download | Focus |
|:-----|:---------|:------|
| **The Most Dangerous Game** | [PDF](/downloads/NC_MDG.pdf) | Rainsford's moral dilemma |
| **The Interlopers** | [PDF](/downloads/NC_Interlopers.pdf) | The feud's psychological impact |
| **The Outsider** | [PDF](/downloads/NC_Outsider.pdf) | Search for identity |
| **Romeo and Juliet** | [PDF](/downloads/NC_Romeo_Juliet.pdf) | Tragic choices |

---

## Templates

| Resource | Description |
|:---------|:------------|
| [Historical Framework Template](/downloads/NC_Historical_Template.pdf) | Adaptable for historical figures and events |

---

## What's in Each Module

1. **Character Profiles** — Who each character is at the moment, with textual evidence for what they want and fear
2. **Threshold Moments** — Critical turning points, chosen because the character could genuinely have gone either way
3. **Lens Prompts** — What each of the three lenses asks at that particular moment
4. **Implementation Guidelines** — Setup, timing, and facilitation notes

---

## Creating Your Own

See the [Modules guide]({% link practice/modules/index.md %}) for instructions on developing custom modules for your texts.

If you create a module you'd like to share, contact [geoffrey@geoffreysperl.com](mailto:geoffrey@geoffreysperl.com) or submit a pull request on [GitHub](https://github.com/geoffreysperl/narrative-crossroads).
